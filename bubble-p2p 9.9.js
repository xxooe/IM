...
class BubbleP2P {
  ...
  /** 把一个JSON字符串发出去：够小就直接发；太大就拆成多片，配合背压
      ...
      换不来什么实际收益）。 */
  async _sendRaw(channel, str) {
    if (str.length <= this.CHUNK_SIZE) {
      await this._sendWithBackpressure(channel, str);
      return;
    }
    const xferId = 'x_' + Date.now().toString(36) + Math.random().toString(36).slice(2);
    const total = Math.ceil(str.length / this.CHUNK_SIZE);
    await this._sendWithBackpressure(channel, JSON.stringify({ kind: 'p2p-chunk-begin', xferId, total }));
    for (let i = 0; i < total; i++) {
      const piece = str.slice(i * this.CHUNK_SIZE, (i + 1) * this.CHUNK_SIZE);
      await this._sendWithBackpressure(channel, JSON.stringify({ kind: 'p2p-chunk', xferId, seq: i, data: piece }));
    }
    await this._sendWithBackpressure(channel, JSON.stringify({ kind: 'p2p-chunk-end', xferId }));
  }
  ...
  _sendWithBackpressure(channel, str) {
  ...
  async sendViaChannel(peerUid, payload) {
    ...
    const entry = this.peers[peerUid];
    try { await this._sendRaw(entry.channel, JSON.stringify(payload)); entry.lastActivityTs = Date.now(); return true; }
  ...
  sendReliable(peerUid, payload) {
    ...
    return (async () => {
      try { await this._sendRaw(entry.channel, str); entry.lastActivityTs = Date.now(); }
      catch (e) { console.warn('[BubbleP2P] 直连发送失败', e); return false; }
